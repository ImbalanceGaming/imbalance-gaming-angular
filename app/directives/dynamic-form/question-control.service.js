        ...
        questions.forEach(function (question) {
            if (question.controlType === 'dropdown') {
                ...
                question.options.forEach(function (option) {
                    if (option.selected) {
                        group[question.key] = question.required ? [option.value || '', forms_1.Validators.required] : option.value || '';
                    }
                });
            }
            else {
                group[question.key] = question.required ?
                    new forms_1.FormControl(question.value || '', forms_1.Validators.required) : new forms_1.FormControl(question.value || '');
            }
        });
